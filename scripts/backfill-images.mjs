...
function resolveNationalityImage(nationality) {
  const code = NATIONALITY_CODES[String(nationality ?? '').trim()] ?? 'un'
  return `https://flagcdn.com/w160/${code}.png`
}

const dbPath = path.join(process.cwd(), 'nba_2022.db')
const SQL = await initSqlJs({
  locateFile: (file) => path.join(process.cwd(), 'node_modules', 'sql.js', 'dist', file),
})
const database = new SQL.Database(fs.readFileSync(dbPath))

const rows = database.exec('SELECT id, college, nationality FROM starters')[0]
let updated = 0

for (const [id, college, nationality] of rows.values) {
  const collegeImage = resolveCollegeImage(college)
  const nationalityImage = resolveNationalityImage(nationality)
  database.run('UPDATE starters SET college_image = ?, nationality_image = ? WHERE id = ?', [
    collegeImage,
    nationalityImage,
    id,
  ])
...
console.log(`Updated ${updated} player rows with college_image and nationality_image.`)
